...
1. **미디어 탭 채널 편집 기능** (관리자 메뉴에서 유튜브/뉴스 채널 추가·삭제·수정)
2. **증시 구조 변경**: ETF 섹션 → "내 연금" 섹션으로 교체
   - 주식 관련 AI멘트 / 연금 관련 AI멘트를 분리해서 각각 추가하는 구조로 변경
3. **포트폴리오 GS 연동** 시간 되면 검토 (현재 완성도 40%, 프론트 그래프 미연결 상태)
4. **PWA 사용법 확인**: 실기기에서 "홈 화면에 추가" 테스트 — 매니페스트/서비스워커는 코드로 추가했지만 실제 설치 동작은 검증 안 됨
5. **실사용 확인 대기**: 내일 이후 cron-job.org 스케줄대로 실제 텔레그램/이메일이 중복 없이 1번만 오는지 재확인 (오늘은 수동 테스트로만 검증함)
