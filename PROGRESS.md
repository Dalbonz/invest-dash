...
- **포트폴리오 구글시트 연동 구조적 버그 발견 및 재구축** (가장 큰 작업)
  - 원인 1: `buildOwnerData`가 항상 브라우저 localStorage를 구글시트보다 우선해서 보여주던 버그 → 시트 연동이 설정되어 있으면 시트가 우선, 안 되어 있을 때만 로컬 캐시로 폴백하도록 수정
  - 원인 2(더 큰 문제): 포트폴리오 탭이 쓰던 Google Apps Script가 `주식현황상세`/`예적금내역`/`입출금통장`(달봉즈님이 실제로 관리하는 시트)을 전혀 안 읽고, `Stocks`/`Cash`/`Savings`라는 별도의 빈 자동생성 탭만 읽고 쓰고 있었음 — 두 시스템이 완전히 분리되어 있던 것
  - 조치: Apps Script를 재작성해서 실제 시트 3개를 직접 읽어 `ref`로 같이 반환하도록 변경 (시트 자체는 안 건드림, 웹 편집용 Stocks/Cash/Savings는 그대로 유지)
  - 웹사이트에 **"시트에서 가져오기"** 버튼 추가 — 처음 한 번 누르면 시트의 기존 데이터(주식 16종목 등)로 웹 편집 화면을 채워줌
  - 이후 웹 값과 시트(`ref`) 값이 다르면 종목/현금/예적금 항목에 빨간 점(●)으로 표시 (자동 덮어쓰기 없음, 차이만 시각적으로 알려줌)
  - **확인 필요**: 예적금내역 탭이 현재 비어있음(실제로 예적금이 없는 건지 확인 필요), "시트에서 가져오기" 버튼 실사용 테스트 아직 안 함
...
1. **안드로이드 실기기에서 오늘(2026-06-21) 변경사항 전체 검증** — 다음 세션 예정
   ...
   - 포트폴리오 탭: "시트에서 가져오기" 버튼으로 16종목 정상 import되는지, 그 후 시트를 고쳤을 때 빨간 점이 뜨는지
2. **미디어 탭 채널 편집 기능** (관리자 메뉴에서 유튜브/뉴스 채널 추가·삭제·수정)
3. **PWA 사용법 확인**: 와이프 폰 등 다른 기기에서도 동일하게 설치되는지 추가 확인 (본인 폰은 설치 성공 확인됨)
4. **실사용 확인**: cron-job.org 스케줄대로 텔레그램/이메일이 중복 없이 1번만 오는지 며칠 더 지켜보기
