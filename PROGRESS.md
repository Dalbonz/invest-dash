...
1. **안드로이드 실기기에서 2026-06-21 변경사항 전체 검증** — 아직 미완료
   - 모바일 카드 헤더 버튼 줄바꿈 수정 확인 (PWA 캐시 때문에 안 보이면 앱 완전 종료 후 재실행 또는 재설치)
   - 탭 간 가로폭 통일 확인
   - 연금 시세 위젯: 종목 추가 → 실시간 가격 정상 조회되는지
   - 포트폴리오 탭: "시트에서 가져오기"로 16종목 import → 그래프/요약 정상 표시되는지, "구글시트에 백업" 버튼 동작 확인
   - 방금 추가한 편집화면 빨간색 불일치 표시도 같이 확인
2. **구글시트 셀 개수 한도 정리** — `_RAW`로 끝나는 트렌드 추적 탭들의 빈 행/열 또는 누적 데이터 정리 (워크북 전체가 1,000만 셀 한도에 걸려 쓰기가 실패하는 상태)
3. **미디어 탭 채널 편집 기능** (관리자 메뉴에서 유튜브/뉴스 채널 추가·삭제·수정)
4. **PWA 사용법 확인**: 와이프 폰 등 다른 기기에서도 동일하게 설치되는지 추가 확인 (본인 폰은 설치 성공 확인됨)
5. **실사용 확인**: cron-job.org 스케줄대로 텔레그램/이메일이 중복 없이 1번만 오는지 며칠 더 지켜보기
